Basic Calculator Web App

A simple and responsive Basic Calculator built using HTML, CSS.
This project performs standard arithmetic operations with a clean and user-friendly interface.

📌 Project Overview
This calculator web application allows users to perform basic mathematical calculations such as:
Addition
Subtraction
Multiplication
Division
It is designed for beginners to understand DOM manipulation, event handling, and basic UI styling.

🛠️ Technologies Used
HTML5 – Structure of the calculator
CSS3 – Styling, layout, and responsiveness


✨ Features
Interactive calculator buttons
Clean and modern UI
Responsive grid-based layout
Hover effects for better UX
Clear (C) button to reset input
Supports decimal values

👩‍💻 Author
Monika Kumari
Aspiring Web Developer


⭐ Acknowledgement
This project was created as part of learning front-end web development and practicing JavaScript fundamentals.
